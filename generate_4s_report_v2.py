...
function updateQueryView_{html_id}() {{
  ...
  queryItems.forEach(item => {{
    ...
    // Top-40 rank information
    const inControlTop40 = item.in_control_top40 || false;
    const controlRank = item.control_rank || null;
    const inVariantTop40 = item.in_variant_top40 || false;
    const variantRank = item.variant_rank || null;

    let top40Badge = '';
    if (inVariantTop40 && !inControlTop40) {{
      // High impact: appeared in variant top-40
      top40Badge = '<span style="background: #10b981; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; margin-left: 8px;">🎯 NEW IN TOP-40</span>';
    }} else if (!inVariantTop40 && inControlTop40) {{
      // High impact loss: disappeared from top-40
      top40Badge = '<span style="background: #ef4444; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; margin-left: 8px;">⚠️ LEFT TOP-40</span>';
    }} else if (inVariantTop40 && inControlTop40) {{
      // Medium impact: in top-40 for both
      top40Badge = '<span style="background: #f59e0b; color: white; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; margin-left: 8px;">📊 IN TOP-40</span>';
    }}

    let rankInfo = '';
    if (controlRank !== null || variantRank !== null) {{
      rankInfo = '<div style="margin-top: 8px; padding: 8px; background: #f3f4f6; border-radius: 4px; font-size: 12px;">';
      if (controlRank !== null) {{
        rankInfo += `<span style="margin-right: 12px;"><strong>Control Rank:</strong> #${{controlRank}}</span>`;
      }} else {{
        rankInfo += '<span style="margin-right: 12px; color: #9ca3af;"><strong>Control:</strong> Not in top-40</span>';
      }}
      if (variantRank !== null) {{
        rankInfo += `<span><strong>Variant Rank:</strong> #${{variantRank}}</span>`;
      }} else {{
        rankInfo += '<span style="color: #9ca3af;"><strong>Variant:</strong> Not in top-40</span>';
      }}
      rankInfo += '</div>';
    }}
...
def main():
    ...
    if '_qip_pairs_with_top40_with_pgls.parquet' in item_attrs_path:
        ...
        item_attrs_path = item_attrs_path.replace('_qip_with_top40.parquet', '_item_attributes.jsonl')
    elif '_qip_pairs.parquet' in item_attrs_path:
